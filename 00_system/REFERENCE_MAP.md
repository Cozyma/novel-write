...
- **必読（親セッション＝設計・判定記帳役）**: `_workspace/canon/` 3本（憲章=最上位／骨格／戦闘カノン）＋ `_workspace/probes/arc01_probes.md`（試作台帳＝判定ログ・注意台帳。**親専用**）＋ `ARCHITECTURE.md` 2.4（決定の枠組み・設計記述の規律＝身分ラベル・検証条件の事前登録・**案提示の前提1行明示**。2026-07-14配線）
...
### ケースH: 管理計画レビュー（方針・目標スタック・作業順序。2026-07-14実績）
- **必読**: `WORK_MASTER.md` §2〜§3 / `CONTEXT.md`直近 / `TODO.md`対象項目 / `ARCHITECTURE.md`運用フック
- **必要時**: `SELF_REVIEW.md` §1〜§2（ルール・テンプレートを計画に含む場合）/ 計画が根拠として明示する成果物のみ
- **読まない**: 作品実データ・蒸留元・旧稿——計画上の主張を検証する必要が生じた箇所だけ追加参照する
- **判定**: 目標整合／順序依存／スコープ境界／充足条件の観測可能性／正本と還流方向。レビュー中は実装せず、指摘と保留を `CONTEXT` / `TODO` へ同期する
